...
サブタイトル、1968年前後に生まれた学年の、語らいの場
運営者、油谷和好
ドメイン、1968.love
GitHubリポジトリ、https://github.com/kazuyoshiAburatani/1968.git

1968.LOVE は、1968年に生まれた学年（昭和43年度生まれ）を**中心に**、
そのひとつ上（昭和42年度）とひとつ下（昭和44年度）までが集まる場。
同じ校舎で同じテレビの話をしていた人にしか通じない話を、短い言葉で交わすためのサービス。
...
- ターゲット、昭和42年度・43年度・44年度生まれの3学年（**1967年4月2日〜1970年4月1日生まれ**）
- 中心はあくまで昭和43年度生まれ（1968年4月2日〜1969年4月1日）
- 収益モデル、当面なし。全機能無料
- 想定会員数、初年度500名（mixi の生まれ年コミュニティが 20 年で数百〜数千人規模なので妥当な線）

### 対象を3学年にした理由（2026-08-10）

当初は昭和43年度の 1 学年だけだった。これを、中心は据え置いたまま
ひとつ上（昭和42年度）とひとつ下（昭和44年度）まで広げた。

- 1 学年だけだと母数が薄く、立ち上げ期にいちばん危険な「回答 0 件」が人目に触れやすい
- 前後 1 学年は、小学校から高校までずっと同じ校舎にいた相手なので、同じ話がそのまま通じる
- 広告のターゲット年齢も 57〜58 歳ちょうどから 56〜59 歳に広げられ、配信効率が上がる
  （当たりは 62% → 75%）

**ただし「56歳から59歳の方どうぞ」に均さないこと。**
密度が消えると、二択が初回参加 8.8 を取れた理由そのものが無くなる。
書き方の順番は必ず「中心は1968年度、その前後も同じ話が通じるので一緒にどうぞ」にする。
前後の学年に対して「あなたは中心ではない」と読める書き方は禁止（`cohortNote` にテストがある）。
...
- 受け入れ範囲を **1967年4月2日〜1970年4月1日** に拡張
- 学年（年度）は `profiles.school_year` 生成列と `lib/school-year.ts` の両方で計算する。
  **式が 2 か所にあるので、片方だけ直さないこと**
- 受け入れ範囲の判定も 2 か所にある。`lib/school-year.ts` の `isAcceptedBirthday` と、
  DB の `profiles_birth_date_range_check`。どちらも
  `年*10000 + 月*100 + 日` を 19670402〜19700401 で挟む同じ形で書いてある。
  **片方だけ直すと、画面の検証は通るのに保存で落ちる**
...
birth_year の制約は `profiles_birth_date_range_check`、1967年4月2日〜1970年4月1日。
